...
import { getDefaultParamsForType, type PlaceConfigSchema } from './placeConfig';
...
interface NetState {
  ...
  // Edge control point offsets (for draggable edges)
  ...
  // Place operations
  addPlace: (type: string, position: { x: number; y: number }, placeConfig: PlaceConfigSchema) => string;
  ...
  // Edge offset updates (for draggable edges)
...
export const useNetStore = create<NetState>((set, get) => ({
  ...
  addPlace: (type, position, placeConfig) => {
    ...
    const defaultParams = getDefaultParamsForType(placeConfig, type);
  ...
  updatePlace: (id, updates) => {
    set((state) => {
      ...
      if (updates.id && updates.id !== id) {
        ...
        // Update edge offsets to use new ID
        const newEdgeOffsets: Record<string, { x: number; y: number }> = {};
        for (const [edgeId, offset] of Object.entries(state.edgeOffsets)) {
          // Edge IDs are in format "source->target" or "place::subplace->target"
          const newEdgeId = edgeId
            .replace(`->${oldId}`, `->${newId}`)
            .replace(`${oldId}->`, `${newId}->`)
            .replace(`${oldId}::`, `${newId}::`);
          newEdgeOffsets[newEdgeId] = offset;
        }
  ...
  updateTransition: (id, updates) => {
    set((state) => {
      ...
      if (updates.id && updates.id !== id) {
        ...
        const newEdgeOffsets: Record<string, { x: number; y: number }> = {};
        for (const [edgeId, offset] of Object.entries(state.edgeOffsets)) {
          // Edge IDs are in format "source->target"
          const newEdgeId = edgeId
            .replace(`->${oldId}`, `->${newId}`)
            .replace(`${oldId}->`, `${newId}->`);
          newEdgeOffsets[newEdgeId] = offset;
